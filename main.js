...
const isSnakeOverlappingWithFood = function (state) {
    let head = state.snakeSegements[0];
    let food = state.food;
...
const canGenerateFood = function (state) {
...
const elButtonRestart = document.getElementById('buttonRestart');
let defaultState = {
    x: 0,
    y: 0,
...
function putToLocalStorate(state, key = 'state') {
...
const handleEvent = function (direction) {
    ...
    if (state.isGameOver)
        return;
    let head = deepCopy(state.snakeSegements[0]);
